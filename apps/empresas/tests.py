...
class SincronizacaoClientesOmieTests(TestCase):
    ...
    def test_consultas_novas_usam_os_contratos_da_omie(self, urlopen_mock):
        ...
        requisicao_titulos = urlopen_mock.call_args_list[6].args[0]
        payload_titulos = json.loads(requisicao_titulos.data)
        self.assertTrue(
            requisicao_titulos.full_url.endswith("/financas/pesquisartitulos/")
        )
        self.assertEqual(payload_titulos["call"], "PesquisarLancamentos")
        self.assertEqual(
            payload_titulos["param"][0],
            {
                "nPagina": 8,
                "nRegPorPagina": 500,
            },
        ...
        requisicao_produtos = urlopen_mock.call_args_list[7].args[0]
        ...
        self.assertEqual(payload_pedidos_compra["call"], "PesquisarPedCompra")
        self.assertEqual(
    ...
    def test_salva_pedido_de_compra_omie_com_itens(self):
        fornecedor = CadastroOmie.objects.create(
            empresa=self.empresa,
            codigo_cliente_omie=7232170609,
            tipo=CadastroOmie.Tipo.FORNECEDOR,
            razao_social="Fornecedor compra",
        )
        ...
        conta = ContaCorrenteOmie.objects.create(
            empresa=self.empresa,
            codigo_omie=6904753294,
            codigo_integracao="",
            descricao="Conta compras",
        )
        produto = ProdutoOmie.objects.create(
            empresa=self.empresa,
            codigo_produto=7186847863,
            codigo="PRD00159",
            descricao="Fonte 12V3A",
        )

        processados = _salvar_pedidos_compra(
            self.empresa,
            [
                {
                    "cabecalho_consulta": {
                        ...
                        "cNumPedido": "",
                        "cNumero": "119",
                        "cObs": "FAVOR COLOCAR O NUMERO DO PEDIDO DE COMPRA NA NOTA FISCAL.",
                        "cObsInt": "",
                        "dDtPrevisao": "11/08/2026",
                        "dIncData": "04/08/2026",
                        "nCodCC": 6904753294,
                        "nCodCompr": 7225449418,
                        "nCodFor": 7232170609,
                        "nCodIntCC": "",
                        "nCodPed": 7232259056,
                        "nCodProj": 0,
                    ...
                    "produtos_consulta": [
                        {
                            "cCodCateg": "2.01.01",
                            "cCodIntItem": "",
                            "cDescricao": "Fonte 12V3A para Firewall Fortinet Fortigate 40F Entrada 127V/220V",
                            "cEAN": "",
                            "cNCM": "8504.40.21",
                            "cObs": "FAVOR ENTREGAR NO ENDERECO",
                            "cProduto": "PRD00159",
                            "cUnidade": "UN",
                            "codigo_local_estoque": "6906334225",
                            "nCodItem": 7232360776,
                            "nCodProd": 7186847863,
                            "nDesconto": 0,
                            "nDespesas": 0,
        ...
        self.assertEqual(processados, 1)
        pedido = PedidoCompraOmie.objects.get(codigo_pedido=7232259056)
        self.assertEqual(pedido.numero_pedido, "119")
        ...
        self.assertEqual(pedido.data_previsao.isoformat(), "2026-08-11")
        self.assertEqual(str(pedido.valor_total_pedido), "208.0000")
        self.assertEqual(pedido.parcelas_consulta[0]["nValor"], 208)
        item = PedidoCompraItemOmie.objects.get(codigo_item=7232360776)
        self.assertEqual(item.pedido, pedido)
        self.assertEqual(item.produto, produto)
        self.assertEqual(item.categoria_principal, categoria)
    ...
    def test_importa_dados_omie_incluindo_contas_correntes(
        ...
        consultar_pesq_titulos_financeiros_mock.return_value = {
            "nPagina": 1,
            "nTotPaginas": 1,
            "nRegistros": 1,
            "nTotRegistros": 1,
            "titulosEncontrados": [
                {
                    "cabecTitulo": {
                        ...
                        "nCodTitulo": 1344090316,
                        "nValorTitulo": 10000,
                        "observacao": "Conta a pagar importada automaticamente.",
                    },
                    "departamentos": [
                        {
                            "cCodDepartamento": "303",
        ...
        consultar_pedidos_compra_mock.return_value = {
            "nPagina": 1,
            "nTotPaginas": 1,
            "nRegistros": 0,
            "nTotRegistros": 0,
        ...
        titulo_pesquisado = PesqTituloFinanceiroOmie.objects.get(
            codigo_titulo=1344090316
        )
        self.assertEqual(titulo_pesquisado.cliente_fornecedor.codigo_cliente_omie, 202)
        self.assertEqual(titulo_pesquisado.conta_corrente, conta)
        self.assertEqual(titulo_pesquisado.categoria_principal, categoria)
        self.assertEqual(titulo_pesquisado.natureza, "P")
        self.assertEqual(titulo_pesquisado.status, "PAGO")
        self.assertFalse(titulo_pesquisado.liquidado)
        self.assertEqual(titulo_pesquisado.numero_documento_fiscal, "NF500013")
        self.assertEqual(str(titulo_pesquisado.valor_titulo), "10000.00")
        self.assertEqual(str(titulo_pesquisado.valor_aberto), "5000.00")
        self.assertEqual(str(titulo_pesquisado.valor_pago), "5000.00")
        self.assertEqual(titulo_pesquisado.lancamentos[0]["nValLanc"], 5000)
